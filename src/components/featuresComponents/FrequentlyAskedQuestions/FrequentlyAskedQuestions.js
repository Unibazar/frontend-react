...
const FrequentlyAskedQuestions = () => {
  ...
  return (
    <div className="w-full  flex-col justify-center pb-10 ">
      <h2 className="w-full font-normal text-center text-2xl py-10">
      ...
      <div className="w-full flex md:flex-row flex-col md:gap-20 gap-4 justify-center">
        <div className="w-full md:w-1/2 flex-col flex gap-4 ">
          {Faq.slice(0, 3).map((faq, index) => (
            <div key={index} className="w-full flex flex-row bg-gray-200 justify-between md:gap-20 gap-4 py-2 px-2 rounded" onClick={() => handleClick(index)}>
              <div className="w-full flex-col" >
                <p className="w-full font-normal text-left ">
                  {index === 0 ? "What is the purpose of this website?" : 
                   index === 1 ? "How can I search for available land?" : 
                   "What payment methods do you accept?"}
                </p>
                {expandedIndex === index && (
                  <p className="w-full font-light text-left text-gray-500">
                    {faq.answer}
                  </p>
                )}
              </div>
              {expandedIndex === index ? (
                <AiOutlineMinus className="w-auto" />
              ) : (
                <AiOutlinePlus className="w-auto" />
              )}
            </div>
          ))}
        </div>
        <div className="w-full md:w-1/2 flex-col flex gap-4">
          {Faq.slice(3, 6).map((faq, index) => (
            <div key={index} className="w-full flex flex-row bg-gray-200 justify-between md:gap-20 gap-4 py-2 px-2 rounded" onClick={() => handleClick(index + 3)}>
              <div className="w-full flex-col" >
                <p className="w-full font-normal text-left ">
                  {index === 0 ? "Is financing available for land purchases?" : 
                   index === 1 ? "Can I visit the land before purchasing?" : 
