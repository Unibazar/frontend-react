      ...
      <form className="w-full p-7" onSubmit={(e)=>handleSubmit(e)}>
          ...
          <div className="flex gap-7 flex-wrap">
            <div className="flex flex-1 flex-col gap-2">
              ...
              <input id="userName" type="text" placeholder="Add Product Name" name="userName" className="border-2 text-sm md:text-lg p-2 rounded-lg" />
            </div>
            <div className="flex flex-1 flex-col gap-2">
              <label htmlFor="phoneNumber">Phone Number:</label>
              <input id="phoneNumber" type="number" placeholder="Phone" name="phoneNumber" className="border-2 text-sm md:text-lg p-2 rounded-lg" />
            </div>
          </div>
          <div className="flex gap-7 flex-wrap mt-5">
            <div className="flex flex-1 flex-col gap-2">
              <label htmlFor="location">Location:</label>
              <input id="location" type="text" placeholder="Enter Address" name="location" className="border-2 text-sm md:text-lg p-2 rounded-lg" />
            </div>
            <div className="flex flex-1 flex-col gap-2">
              <label htmlFor="email">Email:</label>
              <input id="email" type="email" placeholder="Enter Your Email" name="email" className="border-2 text-sm md:text-lg p-2 rounded-lg" />
            </div>
          </div>

          <div className="flex flex-col gap-2 flex-1 w-full mt-7">
            <label htmlFor="bio">Biography:</label>
            <textarea id="bio" placeholder="Write Something About Yourself" name="bio" rows={6} className="border-2 text-sm md:text-lg p-2 rounded-lg"></textarea>
          </div>
